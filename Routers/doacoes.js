...
router.post('/api/doacoes', upload.single('comprovante'), (req, res) => {
  ...
  if (tipo_doacao === 'dinheiro' && !id_projeto) {
    return res.status(400).json({ error: 'Doações em dinheiro precisam estar vinculadas a um projeto.' });
  }
    ...
    (err, result) => {
      ...
      if (tipo_doacao === 'dinheiro' && id_projeto) {
        ...
        mysql.query(update, [valor_doacao, id_projeto], (updateErr) => {
          ...
          return res.status(201).json({ message: 'Doação registrada com sucesso e projeto atualizado.' });
        });
      } else {
        return res.status(201).json({ message: 'Doação registrada com sucesso.' });
      }
    }
  );
});
...
router.get('/api/doacoes', (req, res) => {
  mysql.query('SELECT * FROM tb_doacoes', (err, results) => {
    if (err) return res.status(500).json({ error: err });
    res.json(results);
  });
});
...
router.get('/api/doacoes/:id', (req, res) => {
  const { id } = req.params;
  mysql.query('SELECT * FROM tb_doacoes WHERE id_doacoes = ?', [id], (err, results) => {
    if (err) return res.status(500).json({ error: err });
    if (results.length === 0) return res.status(404).json({ error: 'Doação não encontrada.' });
    res.json(results[0]);
  });
});
