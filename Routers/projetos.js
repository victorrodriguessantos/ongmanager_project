...
router.post("/api/projetos", async function (request, response, next) {
  try {
    ...
    const { name_projeto, descricao, meta_arrecadacao, status_projeto } =
      request.body;
    ...
    if (!name_projeto || !descricao || !meta_arrecadacao) {
      return response
        .status(400)
        .json({ message: "Os campos obrigatórios precisam ser preenchidos." });
    }
...
router.put("/api/projetos/:id", async function (request, response, next) {
  try {
    ...
    mysql.query(
      query,
      [name_projeto, descricao, meta_arrecadacao, status_projeto, id],
      function (error, data) {
        if (error) {
